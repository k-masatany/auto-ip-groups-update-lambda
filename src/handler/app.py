...
SECURITY_GROUP_TAG_FOR_REGION_HTTP = {
    'Name': 'autoupdate_target', 'AutoUpdate': 'true', 'Protocol': 'http'}
SECURITY_GROUP_TAG_FOR_REGION_HTTPS = {
    'Name': 'autoupdate_target', 'AutoUpdate': 'true', 'Protocol': 'https'}
...
def update_security_groups(new_ranges):
    ...
    region_http_group = get_security_groups_for_update(
        client, SECURITY_GROUP_TAG_FOR_REGION_HTTP)
    region_https_group = get_security_groups_for_update(
        client, SECURITY_GROUP_TAG_FOR_REGION_HTTPS)

    print('Found ' + str(len(region_http_group)) +
          ' lambda_regional HttpSecurityGroups to update')
    print('Found ' + str(len(region_https_group)) +
          ' lambda_regional HttpsSecurityGroups to update')
    ...
    result.append('Updated ' + str(region_http_updated) + ' of ' +
                  str(len(region_http_group)) + ' lambda_regional HttpSecurityGroups')
    result.append('Updated ' + str(region_https_updated) + ' of ' +
                  str(len(region_https_group)) + ' lambda_regional HttpsSecurityGroups')
...
def update_security_group(client, group, new_ranges, port):
            ...
            if permission['FromPort'] <= port and permission['ToPort'] >= port:
                ...
                for range in permission['IpRanges']:
                    ...
                    if new_ranges.count(cidr) == 0:
                        ...
                        print(group['GroupId'] + ": Revoking " +
                              cidr + ":" + str(permission['ToPort']))

                for range in new_ranges:
                    if old_prefixes.count(range) == 0:
                        to_add.append({'CidrIp': range})
                        print(group['GroupId'] + ": Adding " +
                              range + ":" + str(permission['ToPort']))

                removed += revoke_permissions(client,
                                              group, permission, to_revoke)
    ...
    print(group['GroupId'] + ": Added " +
          str(added) + ", Revoked " + str(removed))
